"""
Updated web_app.py with 3 input modes as requested by professor:
1. Exon Mode - Gene + exon numbers with optional positions
2. Transcript Mode - HGVS cDNA notation (c. notation)  
3. Genomic Coordinate Mode - Chromosomal coordinates with build
"""
...
try:
    ...
    MANE_LOAD_ERROR = f"MANE file not found at {mane_path}."
    mane_data = {}
    _mane_gene_index = {}
...
def process_exon_mode(gene_input, cnv_type, first_exon, last_exon, start_pos=None, end_pos=None):
    """
    ...
    # TODO: Implement position-within-exon logic if start_pos/end_pos provided
    # For now, use whole exon boundaries
...
def process_genomic_mode(genomic_input, build, cnv_type, gene_hint=None):
    ...
    if gene_hint:
        ...
        if genes_in_region:
            ...
            return [{"error": f"No genes found in region chr{chromosome}:{start:,}-{end:,}. Please provide a gene symbol as a hint."}]
    ...
    # Don't add ISCN notation - not needed in results
    ...
    if request.method == "POST":
        ...
        try:
            if mode == "exon":
                ...
                result = process_transcript_mode(cdna_notation, cnv_type)
