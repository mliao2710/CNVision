"""
================================================================================
FUNCTIONAL_PREDICTOR.PY - Predict Biological Impact of CNVs
================================================================================

PURPOSE:
--------
This is the CORE ANALYSIS MODULE that predicts whether a CNV will cause a
"frameshift" or "in-frame" effect. This is critical for understanding disease
severity.

BIOLOGICAL IMPACT:
------------------
DNA is read in groups of 3 nucleotides (codons). Each codon codes for one
amino acid in a protein.

FRAMESHIFT EFFECT:
- If CNV length is NOT divisible by 3, it shifts the reading frame
- All downstream codons are misread
- Usually produces non-functional or truncated protein
- Typically MORE SEVERE (e.g., Duchenne muscular dystrophy)

IN-FRAME EFFECT:
- If CNV length IS divisible by 3, reading frame is maintained
- Only the affected region is changed
- Protein may still be partially functional
- Typically LESS SEVERE (e.g., Becker muscular dystrophy)

Example:
  Exon lengths: 150bp + 123bp = 273bp total
  273 ÷ 3 = 91 (divisible by 3) → IN-FRAME deletion
  
  Exon lengths: 150bp + 124bp = 274bp total
  274 ÷ 3 = 91.33 (not divisible) → FRAMESHIFT deletion

CODING CONCEPTS & PRINCIPLES:
------------------------------
1. MODULO OPERATOR (%):
   - Checks if number is divisible by 3
   - total_length % 3 == 0 means divisible by 3 (in-frame)
   - total_length % 3 != 0 means not divisible (frameshift)

2. DATA NORMALIZATION:
   - Handles different input formats (exon numbers vs exon dicts)
   - Converts everything to standardized format with lengths

3. LOOKUP TABLES:
   - Uses MANE data to look up exon lengths
   - Enables calculation even when only exon numbers provided

4. STRING FORMATTING:
   - Creates human-readable consequence descriptions
   - Example: "frameshift deletion (2 exons)"
...
def predict_cnv_effect(
    ...
    """
    Predict the functional consequence of a CNV based on affected exons.
    
    This is the KEY FUNCTION that determines if a CNV causes frameshift or
    in-frame effects. The logic is simple but critical:
    - Sum up lengths of all affected exons
    - If sum is divisible by 3 → in-frame (less severe)
    - If sum is NOT divisible by 3 → frameshift (more severe)
    
    Args:
        exon_hits: List of dictionaries, each containing:
            - 'gene': Gene symbol
            - 'transcript': Transcript ID
            - 'hit_exons': List of affected exons (can be numbers or dicts)
            - 'cnv_type': "deletion" or "duplication"
        mane_data: MANE gene/exon data structure (for looking up exon lengths)
