...
- Accepts three input modes: exon numbers, HGVS cDNA, and genomic coordinates.
...
def parse_cdna_notation(cdna_string):
    ...
    # STEP 1: Parse simple exon-range HGVS forms (gene or transcript prefix).
    ...
    # STEP 2: Parse intronic HGVS range pattern.
...
def cdna_to_genomic(gene, transcript, c_position, mane_data):
    ...
    # STEP 1: Sort exons in transcript exon order.
    exons_sorted = sorted(exons, key=lambda x: x.get('exon', 0))
    
    # STEP 2: Walk cumulative cDNA length until target position is reached.
    cumulative = 0
    for exon in exons_sorted:
        ...
        if cumulative + exon_length >= c_position:
            offset = c_position - cumulative - 1
            
            # Convert cDNA offset to genomic coordinate by strand.
...
def process_transcript_mode(cdna_notation, cnv_type, build="GRCh38"):
    ...
    # STEP 1: Resolve notation CNV type.
    ...
    # STEP 2: Resolve gene/transcript against the selected build.
    mane_data, _mane_gene_index, _transcript_to_gene = get_mane_data(build)
    ...
    # STEP 3: Validate resolved gene/transcript and cDNA bounds.
    if not gene:
        if transcript_input:
    ...
    # STEP 4: Convert cDNA range to genomic coordinates.
    ...
    # STEP 5: Map exons and predict consequence.
...
def process_genomic_mode(genomic_input, build, cnv_type, gene_hint=None):
    ...
    # STEP 2: Resolve target gene (hint or overlap search).
    if gene_hint:
    ...
    # STEP 3: Add boundary warnings for start/stop involvement.
    ...
    # STEP 4: Return intronic result when no exon overlap is found.
    ...
    # STEP 5: Predict frame consequence from mapped hits.
    effects = predict_cnv_effect(hits, mane_data)
