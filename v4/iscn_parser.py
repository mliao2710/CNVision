"""
CNVision ISCN Parser - Parses Microarray Clinical Notation

Technologies: Python 3.10+, regex (pattern matching), coordinate extraction

Core Functions:
- parse_iscn_notation(): extracts genomic data from clinical ISCN format
- find_genes_in_region(): identifies genes overlapping genomic region
- Chromosome normalization and coordinate extraction
"""
    ...
    for gene, transcripts in mane_data.items():
        for transcript, exons in transcripts.items():
            for exon in exons:
                ...
                # NEW: Skip if we don't know the chromosome or coordinates
                ...
                # Check overlap only if chromosomes match
...
def format_iscn_output(
    ...
    """Format CNV results in ISCN notation."""
