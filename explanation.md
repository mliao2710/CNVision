...
├── web_app.py                                    [Flask web application and main request router]
├── coordinate_mapper.py                          [Maps genomic coordinates to exon/CDS overlaps]
├── functional_predictor.py                       [Predicts in-frame vs frameshift consequence]
├── iscn_parser.py                                [Parses ISCN-like inputs and region/gene helpers]
├── mane_loader.py                                [Loads gene/transcript/exon reference data]
├── explanation.md                                [Full technical explanation (this document)]
...
├── templates/
│   └── index.html                                [Web UI template (HTML/CSS/Jinja)]
├── static/
│   └── logo.png                                  [Logo image]
├── data/
│   ├── gene2refseq                               [Gene-to-RefSeq mapping database]
│   ├── GRCh37.ref_seq_select.gz                  [GRCh37 RefSeq annotation dataset]
│   └── MANE.GRCh38.v1.4.refseq_genomic.gff.gz    [MANE GRCh38 RefSeq annotation dataset]
└── __pycache__/
    ├── coordinate_mapper.cpython-314.pyc         [Auto-generated bytecode cache]
    ├── functional_predictor.cpython-314.pyc      [Auto-generated bytecode cache]
    ├── iscn_parser.cpython-314.pyc               [Auto-generated bytecode cache]
    ├── mane_loader.cpython-314.pyc               [Auto-generated bytecode cache]
    └── web_app.cpython-314.pyc                   [Auto-generated bytecode cache]
...
Browser User
    |
    v
Flask Route: index() [web_app.py]
    |
    v
Read form mode + inputs
    |
    +-------------------------------+
    | mode == exon                  |
    | mode == coordinate            |
    ...
    +-------------------------------+
        |             |             |
        v             v             v
   Exon flow     Coordinate flow   HGVS flow
        \             |             /
         \            |            /
          +-----------+-----------+
                      |
                      v
Map overlap to exons/CDS [coordinate_mapper.py]
                      |
                      v
Predict in-frame/frameshift/no-CDS [functional_predictor.py]
                      |
                      v
Render result/error to templates/index.html
```

Mode 1: Exon Number Mode Flow

```text
User enters: gene + first_exon + last_exon + CNV type + build
    |
    v
process_exon_mode(...)
    |
    v
Normalize build + resolve gene/transcript from MANE dataset
    |
    v
map_exon_numbers_to_regions(...) to get genomic span
    |
    v
map_cnv_to_exons(...) to compute CDS overlap lengths
    |
    v
predict_cnv_effect(...)
    |
    v
Return consequence + supporting exon table to UI
```

Mode 2: Genomic Coordinate Mode Flow

```text
User enters: genomic string (+ optional gene hint) + CNV type + build
    |
    v
process_genomic_mode(...)
    |
    v
parse_genomic_input(...) [extract chr/start/end/build/type hints]
    |
    v
If no gene hint: find_genes_in_region(...) [iscn_parser.py]
If gene hint: use provided gene (normalized)
    |
    v
Select transcript and build CNV payload
    |
    v
map_cnv_to_exons(...) [partial/full exon + CDS overlap]
    |
    v
predict_cnv_effect(...)
    |
    v
Return consequence + warnings/notes + detailed overlaps
```

Mode 3: HGVS (g.) Mode Flow

```text
User enters: HGVS genomic notation + optional build/type selections
    |
    v
process_hgvs_mode(...)
    |
...
parse_hgvs_genomic_notation(...)
    |
    v
Extract chr + breakpoints + inferred CNV type + inferred build
    |
    v
Resolve gene/transcript for interval (or overlap candidates)
    |
    v
map_cnv_to_exons(...) for CDS overlap math
    |
    v
predict_cnv_effect(...)
    |
    v
Return consequence + HGVS-derived notes to UI
```
