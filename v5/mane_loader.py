"""
CNVision MANE Loader - Reference Database Parser and Cache

Technologies: Python 3.10+, gzip (compression), regex (parsing), file I/O

Core Functions:
- load_mane_exons(): parses GFF/GTF files into nested dict structure
- load_gene_mapping(): reads NCBI gene2refseq for transcript→gene mapping
- Builds in-memory indices for fast gene/exon lookups
"""
...
def load_mane_exons(gff_path):
    ...
    with gzip.open(gff_path, "rt") as f:
        for line in f:
            ...
            # Only process exon/CDS features
            ...
            # Extract transcript ID. GFF CDS rows often use Parent=rna-NM_xxx
            ...
            # Store exon and CDS data separately, then combine after parsing.
    ...
    for gene in exon_data:
        for transcript in exon_data[gene]:
            ...
            # Attach exon-level CDS segments for coding-aware consequence calling.
