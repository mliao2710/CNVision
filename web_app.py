...
def get_reference_metadata(build):
    """Return versioned reference metadata for the selected genome build."""
    build = normalize_build(build)

    if build == "GRCh38":
        return {
            "genome_build": "GRCh38",
            "reference_annotation": "MANE Release v1.4 (GRCh38.p14; GCF_000001405.40)"
        }

    return {
        "genome_build": "GRCh37",
        "reference_annotation": "RefSeq Select (hg19/GRCh37)"
    }


def add_reference_metadata(results, build):
    """Attach genome-build and reference metadata to successful results."""
    metadata = get_reference_metadata(build)
...
def process_exon_mode(gene_input, cnv_type, first_exon, last_exon, build="GRCh38"):
    ...
    if use_ncbi_data:
        ...
        return add_reference_metadata([result], build)
        ...
        for effect in effects:
            ...
            effect['data_source'] = (
                ...
                if normalize_build(build) == 'GRCh38'
                else 'RefSeq Select (GRCh37)'
            )
        
        return add_reference_metadata(effects, build)
